...
/**
 * Returns a detailed step-by-step breakdown of the Playfair transformation.
 */
export function getPlayfairSteps(text, keyword, mode = "encrypt") {
  if (!text) return { square: [], pairs: [], steps: [] };
  const square = buildKeySquare(keyword || "KEYWORD");
  ...
  const steps = pairs.map(([a, b], index) => {
    const pa = getPos(square, a);
    const pb = getPos(square, b);
    let outA, outB, rule, formula;

    if (pa.row === pb.row) {
      if (mode === "encrypt") {
        outA = square[pa.row * 5 + ((pa.col + 1) % 5)];
        outB = square[pb.row * 5 + ((pb.col + 1) % 5)];
        formula = "Same Row: Shift Right";
      } else {
        outA = square[pa.row * 5 + ((pa.col + 4) % 5)];
        outB = square[pb.row * 5 + ((pb.col + 4) % 5)];
        formula = "Same Row: Shift Left";
      }
      rule = "Row";
    } else if (pa.col === pb.col) {
      if (mode === "encrypt") {
        outA = square[((pa.row + 1) % 5) * 5 + pa.col];
        outB = square[((pb.row + 1) % 5) * 5 + pb.col];
        formula = "Same Column: Shift Down";
      } else {
        outA = square[((pa.row + 4) % 5) * 5 + pa.col];
        outB = square[((pb.row + 4) % 5) * 5 + pb.col];
        formula = "Same Column: Shift Up";
      }
      rule = "Column";
    } else {
      outA = square[pa.row * 5 + pb.col];
      outB = square[pb.row * 5 + pa.col];
      rule = "Rectangle";
      formula = "Rectangle: Swap along row";
    }
    ...
    return {
      ...
      posA: pa,
      posB: pb,
      posOutA: getPos(square, outA),
      posOutB: getPos(square, outB)
    };
  });

  return { square, pairs: pairs.map(p => p.join("")), steps };
}
